...
-- Creates the signals schema and all base tables.
...
CREATE INDEX IF NOT EXISTS idx_crawl_attempts_attempted_at
  ON crawl_attempts(attempted_at);
CREATE INDEX IF NOT EXISTS idx_crawl_attempts_source_attempted_at
  ON crawl_attempts(source, attempted_at);
...
CREATE INDEX IF NOT EXISTS idx_crawl_frontier_run_date ON crawl_frontier(run_date);
...
CREATE INDEX IF NOT EXISTS idx_observation_lineage_run_date ON observation_lineage(run_date);
...
CREATE INDEX IF NOT EXISTS idx_retry_queue_status_due ON retry_queue(status, due_at);
...
CREATE INDEX IF NOT EXISTS idx_contact_research_account ON contact_research(account_id);
...
VALUES (1, 'initial_schema', NOW()::TEXT)
