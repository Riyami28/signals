...
async def _collect_live_news_async(
    ...
    async with httpx.AsyncClient(
        ...
        async def _run_account(account_index: int, account: dict) -> tuple[int, int, int]:
            async with semaphore:
                return await _collect_live_news_account(
                    conn=conn,
                    settings=settings,
                    lexicon_rows=lexicon_rows,
                    account=account,
    ...
    for result in results:
        if isinstance(result, Exception):
            logger.error("collector_worker_failed source=news error=%s", result, exc_info=True)
            failed_workers += 1
            continue
    ...
    logger.info(
        "collection_complete source=news inserted=%d seen=%d failed_workers=%d",
        inserted_total,
        seen_total,
        failed_workers,
    )
