...
async def _collect_live_twitter_account(
    ...
    if use_rapidapi:
        if official_handle:
            ...
            # query mirrors what _rapidapi_search_url actually sends (from:handle (filters))
            query = f"from:{handle_bare} (-is:retweet lang:en)"
            search_url = _rapidapi_search_url(
                rapidapi_host, f"-is:retweet lang:en", from_handle=handle_bare, since_id=since_id
            )
        else:
